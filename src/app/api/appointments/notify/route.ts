        ...
        if (type === 'new') {
            ...
            if (managers && managers.length > 0) {
                ...
                const managerMsg = appointments.length === 1
                    ? `📅 New Booking! ${customer?.name || 'Customer'} booked ${aptSummary[0]} on ${shortDate}. - ${SALON_NAME}`
            ...
            // Customer SMS
            if (customer?.phone) {
                const msg = `🔄 Appointment Rescheduled! Your ${serviceNames} appointment has been moved to ${shortDate} at ${appointment.start_time}. See you then! - ${SALON_NAME}`;
                const result = await textlk.sendSMS(customer.phone, msg);
                results.customer = result;
            }
            ...
            if (managers && managers.length > 0) {
                const managerMsg = `🔄 Appointment Rescheduled! ${customer?.name || 'Customer'}'s ${serviceNames} moved from ${oldDateStr} ${oldTimeStr} to ${shortDate} ${appointment.start_time}. - ${SALON_NAME}`;
