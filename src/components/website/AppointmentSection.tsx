...
export default function AppointmentSection({ isStandalone = false }: AppointmentSectionProps) {
    ...
    const validatePhone = (phone: string): string | undefined => {
        ...
        // Extract local digits (strip +94 prefix)
        const digits = phone.replace(/\D/g, '').replace(/^94/, '');
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="max-w-md mx-auto space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex">
                                    <span className="flex items-center px-3 py-3.5 bg-[var(--t-bg-2)] border border-r-0 border-[var(--t-border)] text-sm font-medium text-[var(--t-text-2)] select-none whitespace-nowrap">
                                        🇱🇰 +94
                                    </span>
                                    <input
                                        type="tel"
                                        value={customer.phone.replace(/^\+94/, '')}
                                        onChange={(e) => {
                                            let digits = e.target.value.replace(/\D/g, '');
                                            if (digits.startsWith('0')) digits = digits.slice(1);
                                            digits = digits.slice(0, 9);
                                            const fullPhone = digits ? `+94${digits}` : '';
                                            setCustomer(prev => ({ ...prev, phone: fullPhone }));
                                            if (showErrors) setFieldErrors(prev => ({ ...prev, phone: validatePhone(fullPhone) }));
                                        }}
                                        onBlur={() => { if (customer.phone) { setFieldErrors(prev => ({ ...prev, phone: validatePhone(customer.phone) })); setShowErrors(true); } }}
                                        placeholder="701234567"
                                        maxLength={9}
                                        inputMode="numeric"
                                        className={`flex-1 p-3.5 bg-[var(--t-bg-2)] border text-[var(--t-text)] placeholder-[var(--t-text-3)] focus:outline-none transition-all text-sm ${showErrors && fieldErrors.phone ? 'border-red-400 focus:border-red-500' : 'border-[var(--t-border)] focus:border-[var(--t-accent-2)]'}`}
                                    />
                                </div>
                                {showErrors && fieldErrors.phone && <p className="text-red-500 text-xs mt-1">{fieldErrors.phone}</p>}
                                <p className="text-xs text-[var(--t-text-3)] mt-1">Enter 9 digits after +94</p>
                            </div>
